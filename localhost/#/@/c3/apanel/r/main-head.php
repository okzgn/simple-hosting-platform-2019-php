...
<head>
	...
	<link rel="icon" href="$S/i.png">

	<meta charset="UTF-8">
	<meta name="description" content="Panel de administración de $N">
	<meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=1, user-scalable=no">
	<meta name="author" content="OKZGN">

	<meta property="og:title" content="Panel de administración">
	<meta property="og:description" content="Panel de administración de $N">
	<meta property="og:image" content="$S/a.png">
	<meta property="og:url" content="$S/apanel">
...
a, input, select, textarea, button { transition: all 0.125s; }
...
body, select, input, textarea, button { word-spacing: 0.3rem; }
p {
	display: block;
	padding: 0.5rem;
	color: #fff;
...
h1, legend, .L, .l, p, .S b, .O b { text-shadow: 0 1px rgba(0, 0, 0, 0.25); }
*, ::after, ::before {
...
::placeholder, h2 { color: #666; }
h1, h2, h3, h4, h5, h6 { font-size: 100%; }
a, h1 { color: #06f; }
h1, h2 { align-self: center; }
header {
	padding: 0.5rem;
...
h1 svg { height: 100%; }
h1, h2, legend { text-transform: uppercase; }
h2 { text-shadow: 0 1px rgba(0, 0, 0, 0.25), 0 1px #fff, 0 0 0.5rem #fff; }
legend, input[type="submit"], button { font-weight: bold; }
label span {
...
[data-action="submit"]:not(a) {
	cursor: pointer;
	background: #06f;
	color: #fff;
}
[data-action="submit"]:not(a):hover { text-shadow: 0 1px rgba(0, 0, 0, 0.5), 0 0 1rem rgba(255, 255, 255, 0.75), 0 0 1.5rem rgba(255, 255, 255, 0.75); }
[data-action="submit"]:not(a):focus {
	background: #039;
	color: #ccc;
	text-shadow: none;
}
button { background: #ccc; }
button:focus { background: #999; }
input[type="text"]:hover, input[type="password"]:hover, textarea:hover, textarea:focus, input:focus, textarea:focus, select:focus, .S > div, .B > div { box-shadow: inset 0 0 1rem rgba(0, 0, 0, 0.25); }
label {
...
label input[type="text"], label input[type="password"], label input[type="file"], label select, label textarea { width: 100%; }
legend {
...
form iframe, form p span { display: none; }
.l, .L { animation: L 0.5s ease 0s alternate infinite; }
.L {
	color: #000;
	padding-top: 0.5rem;
	text-align: center;
...
.L, .L span { display: block; }
.O { animation: O 0.5s ease 0s normal 1 forwards; }
@keyframes O {
	to {
		opacity: 0;
		height: 0;
...
[data-communicate], [data-communicate] .x {
	width: 100%;
	height: 100%;
}
[data-communicate] .x {
...
.in {
	display: block;
	opacity: 0;
	animation: A 0.125s ease-in-out 0s normal 1 forwards;
}
@keyframes A { to { opacity: 1; } }
.out {
	opacity: 1;
	animation: B 0.125s ease-in-out 0s normal 1 forwards;
}
@keyframes B {
	99% { opacity: 0; }
	to { display: none; }
...
var F = function(i, n, g, e, r, s){
	...
	i.formObj.submit(), i.iframe.hide().off('.F').one('load.F', function(o){
		(o = (i.iframe[0].contentDocument || i.iframe[0].document));
		if(!o || !o.documentElement){ (i.r = '<p data-err>Unexpected error</p>'); }
		else {
			(i.r = o.documentElement.innerHTML), $('head', o).append('<style>' + (i.cssObj || $(i.cssStr || F.cssStr)).html() + '</style>');
			/*if(s = o.documentElement.querySelector('[data-err]')){ $('body', o).prepend(s); }
			else if(s = o.documentElement.querySelector('[data-ok]')){ $('body', o).prepend(s); }
			else { $('body', o).prepend('<p data-err>Intente en otro momento</p>'); }*/
			i.iframe.show();
		}
		i.container.removeClass(g);
		if(i.r.indexOf('Unexpected error') != -1){
			(X.F_outStyles[i.n] = i.outTimeEnd = 1);
			return;
		}
		X.F_outStyles[i.n] = i.outTimeEnd = ((new Date).getTime() + (i.outTime || F.outTime));
		if(typeof i.doneFn == 'function'){ i.doneFn.call(i, i.container, o); }
	});
	return i;
};
(F.n = 0), (F.destinyStr = 'fieldset'), (F.destinyMethod = 'prepend'), (F.containerStr = 'p'), (F.containerHtml = '<p><span>Cargando...</span></p>'), (F.loadStyle = 'L'), (F.cssStr = '#Bs'), (F.outTime = 2567), (X.F_outStyles = function(a){ setInterval(function(){ for(a in X.F_outStyles){ if((new Date).getTime() >= X.F_outStyles[a]){ F[a].container.addClass(F[a].outStyle), (delete X.F_outStyles[a]); } } }, 1000); });
var A = function(x, y){
